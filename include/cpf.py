...
from .cpg import mgmt_descr_by_fqdn, mgmt_get_fqdn_by_name, mgmt_get_fqdn, MyDumper
...
def show_domains(mdm_server: str) -> List[str]:
    if "." in mdm_server:
        client = Mgmt().login( \
            ManagementToLogin(fqdn=mdm_server, dmn="System Data"))[1]
    else:
        client = Mgmt().login( \
            ManagementToLogin(name=mdm_server, dmn="System Data"))[1]
    res = client.api_call("show-domains", {
            "limit" : 250,
            "offset" : 0,
            "details-level" : "standard"})
